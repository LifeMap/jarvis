...
export function createModelRegistry(env: Env): ModelRegistry {
  const workersModel = env.WORKERS_AI_MODEL ?? (env.LLM_PROVIDER === "workers-ai" ? env.LLM_MODEL : DEFAULT_WORKERS_AI_MODEL);
  const openAiModel = env.OPENAI_MODEL ?? (env.LLM_PROVIDER === "openai" ? env.LLM_MODEL : DEFAULT_OPENAI_MODEL);
  const models: RegisteredModel[] = [
    {
      provider: "workers-ai", model: workersModel, enabled: Boolean(env.AI),
      displayName: "Workers AI · Llama 3.3 70B Instruct FP8 Fast",
  ...
  if (env.LLM_PROVIDER === "test") {
    models.push({
      provider: "test", model: env.LLM_MODEL, enabled: true, displayName: `Test · ${env.LLM_MODEL}`,
      capabilities: { toolCalling: true, structuredOutput: true },
    });
  }
  return new ModelRegistry(models, { provider: parseProvider(env.LLM_PROVIDER), model: env.LLM_MODEL });
}
